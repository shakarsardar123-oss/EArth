#!/usr/bin/env python3
"""
Idempotent patch script: robust Gemini base-URL handling.
Safe to re-run — already-applied changes are SKIPPED instead of failing.

Run from the repo root:
    python3 patch_gemini_baseurl_v2.py
"""
...
def patch(path, old, new, count=1):
    ...
    # Already applied? (new text present, old text gone) -> skip quietly.
    if c.count(new) >= count and c.count(old) == 0:
        print(f"SKIP (already applied): {path}")
        return

    n = c.count(old)
    if n != count:
        print(f"FAILED: {path}: expected {count} match(es) of OLD text, found {n}")
        print("---- looking for ----")
        print(old)
        sys.exit(1)
    c = c.replace(old, new)
